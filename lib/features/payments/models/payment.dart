...
/// `Payments` — el `id` ya es el `referenceId` (UUID). `userId`/`professionalId` siguen siendo el
/// Int interno crudo (mismo patrón ya documentado en `Service`, ver `openspec/decisions.md`).
/// `refundDetails.refundedAmount` (si existe) es el acumulado ya reembolsado — no hay un campo
/// `availableForRefund` explícito, se calcula con [amountAvailableForRefund].
class Payment {
  const Payment({
    required this.id,
