...
| I-05 | BAJO | [x] | e96a907, fd15146 | Spec en biometric-login.md (e96a907). Hallazgo: hoy no hay fricción en apertura normal (sesión ya se restaura sola); biométrico solo ayuda post-logout explícito. Implementado en fd15146: opt-in tras login exitoso (nunca en el arranque transparente), switch de solo-desactivar en profile_screen.dart, botón en login_screen.dart. Hallazgo adicional verificado en la práctica: `local_auth` sin canal nativo se cuelga bajo `flutter test` sin lanzar — se le agregó timeout explícito a `BiometricLoginService`, mismo criterio que M-06 |
