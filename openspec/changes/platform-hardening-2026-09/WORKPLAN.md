...
| M-04 | ALTO | [x] | 11a1685 | Script propio (no openapi_generator/swagger_dart_code_generator); PoC en `ratings`, probado contra fixture local — ver CODEGEN.md. Cabo suelto cerrado en 6649ddd/c75cb89/dbd8561: payments/services/professional_profile migrados contra `--openapi-url` real — ver CODEGEN.md §7.1. Continuado 2026-09-11 en 7b42e40 (professional_documents) y 2cd9ae3 (promotions + extensión arrays de number) — contra fixture local, backend no disponible esa sesión, ver CODEGEN.md §7.2. Continuado 2026-09-12 en 3c523ec (generador: soporte de arrays de objetos anidados + 22 tests propios) y 2cc5873 (`contracts` migrado, contra `--openapi-url` de un backend real levantado esta sesión) — ver CODEGEN.md §7.3. **CAMBIO DE ESTRATEGIA 2026-09-12** (mismo día, ver CODEGEN.md §10): con 7 de 72 modelos migrados y 4 tandas de trabajo para llegar a 10-14 tandas necesarias para el resto, José decidió parar la migración dominio por dominio — el valor real nunca fue generar código, fue comparar contra el swagger real. Se construyó en su lugar `dart run tool/openapi_codegen/check_drift.dart`: un verificador de drift que compara CUALQUIER modelo Dart (a mano o generado, sin distinción) contra su schema real, vía un mapeo modelo↔schema versionado (`tool/openapi_codegen/model_mapping.dart`) con exenciones obligatoriamente justificadas. Corrido contra los 72 modelos reales (backend real levantado, `--openapi-url`): primera corrida, 24 hallazgos en 6 dominios. José auditó `LoginResult` (auth) y confirmó que era un falso positivo: `AuthRepository.login()` lo arma a mano y ya traduce correctamente `login`/`requiredNewPassword` (backend) a `success`/`requiresNewPassword` (Dart) — sin campo ignorado ni bug. Registrado en `model_mapping.dart` (rename para el par que coincide en tipo/nulabilidad, exención de campo para el par que además normaliza `null`->`false` a propósito — ver CODEGEN.md §12.4, ejemplo trabajado). Resultado vigente: **20 hallazgos en 5 dominios**, todos campos reales descartados en silencio — `Category` (9), `PaymentMethod` (6), `ServiceClientSummary` (3), `NearbyProfessional` (1), `ProfessionalLastLocation` (1). Ningún fix de estos 20 aplicado todavía, queda por dominio para que José priorice. Limitación anotada explícitamente en CODEGEN.md §11: el verificador no compara valores de enum (hallazgo manual real: `LegalDocumentType` cubre 4 de los 6 valores que ya expone el schema). Ver CODEGEN.md §10-§14 para el diseño completo, el "how to contribute" y el resultado detallado. **Los 20 hallazgos cerrados 2026-09-14** (un commit por dominio, todos a mano): `Category` en 2aeec39 (9 campos; `status` sumó el enum `CategoryStatus`, exento en 206ce37 como enum espejo); `NearbyProfessional`/`ProfessionalLastLocation` en 3b451b4 (`isAvailable` con cambio de UI real — marcador atenuado en el mapa de cercanos; `lastUpdate` sin consumidor); `PaymentMethod` en 94a573e (6 campos; `expiresAt` se muestra en `payment_methods_screen.dart` — decisión pendiente sobre si un método vencido debería poder seleccionarse para pagar, no resuelta); `ServiceClientSummary` en c0d37e9 (`id`/`email`/`phoneNumber`, sin cambio de UI — mostrarle contacto del cliente al profesional es una decisión de producto no tomada). `Category.status`/`isVisible` NO dispararon un filtro nuevo en el cliente: se confirmó que `GET /categories` (único endpoint que Mobile usa para listar) ya filtra `status=ACTIVE, isVisible=true` del lado del backend (`CategoriesService.findAll`) — filtrar de nuevo hubiera sido redundante. `check_drift` vuelve a dar 0 hallazgos contra los 72 modelos (verificado con `--openapi-url` contra un backend real) |
...
| I-01 | CRÍTICO | [x] | 274f947, 400fce5 | Spec en account-deletion.md (274f947), contra el contrato ya definido en TekoApp-Backend (f38ca86). Cabo suelto cerrado en 400fce5: backend implementado (`a24cf7f`), UI real contra `POST /auth/me/deletion-request`+`/cancel` bajo `auth/me` (no `users/me`, la spec asumía el segundo — contrato real verificado). Copy legal tomada de la spec (qué se anonimiza/retiene, ventana de gracia, cancelable), sin inventar texto nuevo — mismo criterio que debe seguir Web contra el mismo contrato |
...
| I-05 | BAJO | [x] | e96a907, fd15146, 2a01885 | Spec en biometric-login.md (e96a907). Hallazgo: hoy no hay fricción en apertura normal (sesión ya se restaura sola); biométrico solo ayuda post-logout explícito. Implementado en fd15146: opt-in tras login exitoso (nunca en el arranque transparente), switch de solo-desactivar en profile_screen.dart, botón en login_screen.dart. Hallazgo adicional verificado en la práctica: `local_auth` sin canal nativo se cuelga bajo `flutter test` sin lanzar — se le agregó timeout explícito a `BiometricLoginService`, mismo criterio que M-06. Config nativa en 2a01885: `MainActivity` necesitaba `FlutterFragmentActivity` (si no, `authenticate()` falla con `no_fragment_activity` — la causa raíz real detrás del cuelgue de arriba, no solo el síntoma) + `USE_BIOMETRIC` en el manifest + `NSFaceIDUsageDescription` en Info.plist (sin ella iOS mata la app al invocar Face ID, mismo tipo de bug que B-02). **Verificación en device real de Android e iOS queda pendiente** (sin dispositivo conectado al momento de ese commit, y sin Mac para iOS — mismo criterio que la verificación pendiente de B-02) |
